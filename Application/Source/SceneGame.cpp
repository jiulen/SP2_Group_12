...
void SceneGame::Init()
{
	...
	meshList[GEO_TREE] = MeshBuilder::GenerateOBJMTL("tree", "OBJ//tree.obj", "OBJ//tree.mtl");
...
void SceneGame::Render()
{
	...
	{
		modelStack.PushMatrix();
		Vector3 targetVector = Vector3(camera.position.x, 0, camera.position.z) - Vector3(-110, 0, 0);
		targetVector = targetVector.Normalized();
		float coinFacing = acosf(Vector3(0, 0, 1).Dot(targetVector));
		if (targetVector.x > 0) {
			coinFacing = Math::RadianToDegree(coinFacing);
		}
		else {
			coinFacing = -Math::RadianToDegree(coinFacing);
		}
		modelStack.Translate(-110, 2.5f, 0);
		modelStack.Rotate(coinFacing, 0, 1, 0);
	...
	{
		modelStack.PushMatrix();
		Vector3 targetVector = Vector3(camera.position.x, 0, camera.position.z) - Vector3(0, 0, -6);
		targetVector = targetVector.Normalized();
		float coinFacing = acosf(Vector3(0, 0, 1).Dot(targetVector));
		if (targetVector.x > 0) {
			coinFacing = Math::RadianToDegree(coinFacing);
	...
	{
		modelStack.PushMatrix();
		Vector3 targetVector = Vector3(camera.position.x, 0, camera.position.z) - Vector3(105, 0, -10);
		targetVector = targetVector.Normalized();
		float coinFacing = acosf(Vector3(0, 0, 1).Dot(targetVector));
		if (targetVector.x > 0) {
			coinFacing = Math::RadianToDegree(coinFacing);
	...
	{
		modelStack.PushMatrix();
		Vector3 targetVector = Vector3(camera.position.x, 0, camera.position.z) - Vector3(0, 0, 80);
		targetVector = targetVector.Normalized();
		float coinFacing = acosf(Vector3(0, 0, 1).Dot(targetVector));
		if (targetVector.x > 0) {
			coinFacing = Math::RadianToDegree(coinFacing);
	...
	{
		modelStack.PushMatrix();
		Vector3 targetVector = Vector3(camera.position.x, 0, camera.position.z) - Vector3(-130, 0, 130);
		targetVector = targetVector.Normalized();
		float coinFacing = acosf(Vector3(0, 0, 1).Dot(targetVector));
		if (targetVector.x > 0) {
			coinFacing = Math::RadianToDegree(coinFacing);
